 ...
 * 
 * Hỗ trợHai công dụng：
 * 1. Quần áo/Trạng thátôi biến thể："mặc hàng ngày"、"đồng phục chiến đấu"、"bị thươngTrạng thái"
 * 2. Tuổi/biến thể sân khấu："phiên bản trẻ"、"phiên bản trung niên"、"Phiên bản người cao tuổi"（với tậpRange）
 */
export interface CharacterVariation {
  id: string;
  name: string;           // "mặc hàng ngày", "đồng phục chiến đấu", "phiên bản trẻ", "phiên bản trung niên" etc.
  ...
  // === Các trường cụ thể của biến thể giai đoạn ===
  isStageVariation?: boolean;      // Cho dù đó là một biến thể sân khấu（tuổi tác/thay đổi thời kỳ）
  episodeRange?: [number, number]; // Phạm vi cài đặt áp dụng：[Tập khởi đầu, Kết thúbộ c]
  ageDescription?: string;         // Tuổi ở giai đoạn này："25 tuổi"、"50 tuổi"
  stageDescription?: string;       // Giai đoạn Mô tả："Giai đoạn đầu kinh doanh"、"đỉnh cao sự nghiệp"
}
...
export interface Character {
  ...
  age?: string;         // tuổi tác/nhóm tuổi
  personality?: string; // Đặc điểm tính cách
  role?: string;        // danh tính/Nền
  traits?: string;      // đặc điểm cốt lõi
  skills?: string;      // Kỹ năng/khả năng
  keyActions?: string;  // việc làm quan trọng
  appearance?: string;  // đặc điểm vật lý
  ...
  tags?: string[];        // Nhân vậthẻ t như #võ thuật #Nam Chúa #kiếm sĩ
