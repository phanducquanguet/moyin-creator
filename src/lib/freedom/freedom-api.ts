 ...
 * Hỗ trợ keyManager：\u9047ĐếnCan Thử lạiLỗi\u65f6đầu tiênKích hoạt key \u8f6e\u6362，\u4e0blầnThử lại\u81ea\u52a8sử dụng\u65b0 key。
 */
async function freedomRetry<T>(
  ...
  for (let attempt = 0; attempt < RETRY_MAX_ATTEMPTS; attempt++) {
    try {
      ...
      // Kích hoạt key \u8f6e\u6362（nếu có keyManager）
...
function pickFeatureConfig(feature: AIFeature, requestedModel?: string): FeatureConfig | null {
  ...
  if (requestedModel) {
    ...
    // UI Mở rộngcủathay đổi\u4f53Mô hình（Chẳng hạn như gemini-3.1-pro từLiên kếtcủa gemini-3-pro Mở rộng\u800c\u6765）\u4e0d\u4f1a
    // \u7cbe\u786etrận đấuĐến\u4efb\u4f55 config.model，\u6b64\u65f6\u56de\u9000Đến\u8f6e\u8be2Cấu hình\u800c\u975eQuay lại null，
    // \u907f\u514dNgười dùng\u9009\u4e86Có sẵnthay đổi\u4f53\u5374\u62a5"Chưa được định cấu hình"Lỗi
  }
...
function detectFreedomImageRoute(model: string, endpointTypes?: string[]): FreedomImageRoute {
  ...
  // Kling image: Mô hình tênPhát hiện + \u7aef\u70b9\u5143\u6570\u636ePhát hiện
...
const FREEDOM_VIDEO_ROUTE_MAP: Record<string, FreedomVideoRoute> = {
  'openAI\u5b98\u65b9VideoĐịnh dạng': 'openai_official',
  'openAIVideoĐịnh dạng': 'openai_official',
  '\u8c46\u5305Video\u5f02\u6b65': 'volc',  // doubao-seedance uses /volc/v1/contents/generations/tasks
  '\u5f02\u6b65': 'wan',
  '\u6587\u751fVideo': 'kling',
  '\u56fe\u751fVideo': 'kling',
  'Videomở rộng': 'kling',
  ...
  '\u5bf9\u53e3\u578b': 'kling',
  'VideoHiệu ứng': 'kling',
  'openai': 'unified', // \u67d0\u4e9bTuỳ chỉnhNhà cung cấp\u628aVideoMô hình\u6807chophổ quát openai
  'Video\u7edfmộtĐịnh dạng': 'unified',
  'grokVideo': 'unified',
  'openai-response': 'unified',
  'biển\u87baVideoTạo': 'unified',
  'lumaVideoTạo': 'unified',
  'lumaVideo\u6269\u5c55': 'unified',
  'runway\u56fe\u751fVideo': 'unified',
  'aigc-video': 'unified',
  'wanVideoTạo': 'unified',  // wan2.6 models use memefast /v1/video/generations
  // Vidu endpoint types (all route to unified /v1/video/generations)
  'vidu\u6587\u751fVideo': 'unified',
  'vidu\u56fe\u751fVideo': 'unified',
  'viduTài liệu tham khảo\u751fVideo': 'unified',
  'vidu\u9996\u5c3e\u5e27': 'unified',
  'lumaVideomở rộng': 'unified',  // luma extend uses mở rộng (file 04 naming)
};

/**
 * \u7edfmộtĐịnh dạng\u7aef\u70b9Đường dẫnmap（\u7aef\u70b9Loại → \u63d0\u4ea4/\u8f6e\u8be2 URL Đường dẫn）
 * \u6bcf\u79cd\u7aef\u70b9Loại\u76f4\u63a5\u5bf9\u5e94\u786e\u5b9acủa URL，\u4e0dMột lần nữa\u9760 fallback \u731c\u6d4b
 */
const UNIFIED_ENDPOINT_PATHS: Record<string, { submit: string; poll: (id: string) => string }> = {
  // Đường dẫn\u5747cho\u57dftên\u6839\u8d77của\u7edd\u5bf9Đường dẫn（\u4e0d\u4f9d\u8d56 /v1/ \u524d\u7f00\u62fc\u63a5）
  'grokVideo':     { submit: '/v1/video/create',      poll: (id) => `/v1/video/query?id=${id}` },
  'Video\u7edfmộtĐịnh dạng': { submit: '/v1/video/create',      poll: (id) => `/v1/video/query?id=${id}` },
  'biển\u87baVideoTạo': { submit: '/minimax/v1/video_generation', poll: (id) => `/minimax/v1/query/video_generation?task_id=${id}` },
  ...
  'lumaVideomở rộng': { submit: '/luma/generations',            poll: (id) => `/luma/generations/${id}` },
  'runway\u56fe\u751fVideo': { submit: '/runwayml/v1/image_to_video', poll: (id) => `/runwayml/v1/tasks/${id}` },
  ...
  'vidu\u6587\u751fVideo':   { submit: '/ent/v2/text2video',       poll: (id) => `/ent/v2/task?task_id=${id}` },
  'vidu\u56fe\u751fVideo':   { submit: '/ent/v2/img2video',        poll: (id) => `/ent/v2/task?task_id=${id}` },
  'viduTài liệu tham khảo\u751fVideo': { submit: '/ent/v2/reference2video',  poll: (id) => `/ent/v2/task?task_id=${id}` },
...
function detectFreedomVideoRoute(model: string, endpointTypes?: string[]): FreedomVideoRoute {
  if (endpointTypes && endpointTypes.length > 0) {
    // ưu tiên：\u5b98\u65b9 Sora -> Kling -> Volc -> Wan -> Replicate -> Unified
    for (const t of endpointTypes) {
...
async function _generateFreedomImageInner(
  ...
  if (!config) {
    ...
    toast.error('sự tự do\u677f\u5757Hình ảnhTạoChưa được định cấu hình：Làm ơn Cài đặtTrung bình Cấu hình「Tấm miễn phí-Hình ảnh」hoặc「Hình ảnhTạo」\u670d\u52a1\u6620\u5c04');
    throw new Error(msg);
  }
  ...
  // Mô hình ID \u76f4\u63a5\u900f\u4f20：UI \u9009của\u5c31\u662fNhà cung cấpnguyên bản ID，không có\u9700\u8f6c\u6362
...
async function generateViaChatCompletions(
  ...
  if (!response.ok) {
    ...
    let msg = `Hình ảnhTạo API Lỗi: ${response.status}`;
  ...
  if (!imageUrl) {
    throw new Error('\u672a\u80fdtừtrò chuyệnphản ứngtrongTrích xuấtHình ảnh URL');
  }
...
async function generateViaMidjourneyEndpoint(
  ...
  // \u57ab\u56fe：base64Array（Hình ảnh\u5f15\u5bfc，Định dạng data:image/png;base64,xxx）
  ...
  // MJ API Thành công\u65f6 code === 1；\u5176\u4ed6\u503cthể hiện API \u5c42Lỗi（\u5373\u4f7f HTTP 200）
  ...
  if (!taskId) throw new Error('Midjourney Quay lại\u7a7aNhiệm vụ ID');
  ...
  for (let i = 0; i < IMAGE_POLL_MAX_ATTEMPTS; i++) {
    ...
    if (status === 'success' || status === 'succeeded' || status === 'completed') {
      ...
      if (!imageUrl) throw new Error('Midjourney Thành công nhưng không Quay lạiHình ảnh URL');
      const mediaId = saveToMediaLibrary(imageUrl, params.prompt, 'ai-image');
...
/**
 * từ model tên\u540e\u7f00Tự động Trích xuất rendering_speed
...
async function generateViaIdeogramEndpoint(
  ...
  // Ideogram \u539f\u751fĐường dẫn：/ideogram/v1/ideogram-v3/generate（\u4e0d\u662f /v1/ideogram-v3/generate）
  const rootBase = getRootBaseUrl(baseUrl);
  ...
  // extraParams Ưu tiên；không có\u5219từ model tên\u540e\u7f00suy luận（e.g. ideogram_generate_V_3_TURBO）
  ...
  if (!imageUrl) throw new Error('Ideogram phản ứng\u672achứaHình ảnh URL');
  const mediaId = saveToMediaLibrary(imageUrl, params.prompt, 'ai-image');
  return { url: imageUrl, mediaId };
}
...
async function generateViaReplicateImageEndpoint(
  ...
  for (let i = 0; i < IMAGE_POLL_MAX_ATTEMPTS; i++) {
    ...
    if (status === 'succeeded') {
      ...
      if (!imageUrl) throw new Error('Replicate Thành công nhưng không Quay lạiHình ảnh URL');
      const mediaId = saveToMediaLibrary(imageUrl, params.prompt, 'ai-image');
      return { url: imageUrl, taskId: String(predictionId), mediaId };
    }
    if (status === 'failed' || status === 'canceled') {
      throw new Error(pollData.error || 'Replicate Hình ảnhTạoThất bại');
    }
  }
  throw new Error('Replicate Hình ảnhTạo\u8d85\u65f6');
}
...
async function _generateFreedomVideoInner(
  ...
  if (!config) {
    ...
    toast.error('sự tự do\u677f\u5757VideoTạoChưa được định cấu hình：Làm ơn Cài đặtTrung bình Cấu hình「Phần video miễn phí」hoặc「VideoTạo」\u670d\u52a1\u6620\u5c04');
    throw new Error(msg);
  }
...
function validateVeoVideoUploads(
  ...
  if (capability.mode === 'none') {
    if (total > 0) throw new Error(`Mô hình ${model} \u4e0dHỗ trợTải lênTệpĐầu vào`);
    return grouped;
  }
  ...
  if (capability.mode === 'single') {
    ...
    if (capability.minFiles > 0 && !file) {
      throw new Error(`Mô hình ${model} \u9700\u8981Tải lên 1 \u5f20Hình ảnh`);
    }
    if (grouped.references.length > 0 || !!grouped.last || (!!grouped.single && !!grouped.first)) {
      throw new Error(`Mô hình ${model} \u4ec5Hỗ trợ 1 \u5f20Hình ảnhĐầu vào`);
    }
    return grouped;
  }
  ...
  if (capability.mode === 'first_last') {
    if (grouped.references.length > 0 || !!grouped.single) {
      throw new Error(`Mô hình ${model} \u4ec5Hỗ trợkhung hình đầu tiên/\u5c3e\u5e27Đầu vào`);
    }
    if (capability.minFiles > 0 && !grouped.first) {
      throw new Error(`Mô hình ${model} \u9700\u8981Tải lênkhung hình đầu tiênHình ảnh`);
    }
    if (!grouped.first && grouped.last) {
      throw new Error(`Mô hình ${model} \u4ec5Tải lên\u5c3e\u5e27không có\u6548，\u8bf7đầu tiênTải lênkhung hình đầu tiên`);
    }
    if (total > capability.maxFiles) {
      throw new Error(`Mô hình ${model} nhấtHỗ trợ 2 \u5f20Hình ảnh（khung hình đầu tiên/\u5c3e\u5e27）`);
    }
    return grouped;
  }
  ...
  if (capability.mode === 'multi') {
    if (!!grouped.single || !!grouped.first || !!grouped.last) {
      throw new Error(`Mô hình ${model} \u4ec5Hỗ trợ\u591aHình ảnh tham khảoĐầu vào`);
    }
    if (grouped.references.length < capability.minFiles) {
      throw new Error(`Mô hình ${model} \u81f3\u5c11\u9700\u8981Tải lên 1 \u5f20Hình ảnh tham khảo`);
    }
    if (grouped.references.length > capability.maxFiles) {
      throw new Error(`Mô hình ${model} nhấtHỗ trợ ${capability.maxFiles} \u5f20Hình ảnh tham khảo`);
    }
    return grouped;
  }
...
function dataUrlToBlob(dataUrl: string, mimeHint?: string): Blob {
  ...
  if (!match) throw new Error('Tải lênTệpĐịnh dạngkhông có\u6548，\u5fc5\u987b\u662f data URL hoặc http(s) URL');
...
async function toUploadBlob(file: FreedomVideoUploadFile): Promise<Blob> {
  if (/^https?:\/\//i.test(file.dataUrl)) {
    ...
    if (!resp.ok) throw new Error(`không có\u6cd5Tải xuốngTải lênChất liệu：${resp.status}`);
    return resp.blob();
  }
...
async function generateVideoViaOpenAIOfficial(
  ...
  if (!taskId) throw new Error('Sora Quay lại\u7a7aNhiệm vụ ID');
...
async function generateVideoViaUnified(
  ...
  if (isVeoModel(model)) {
    ...
    // Reference images: viduTài liệu tham khảo\u751fVideo and similar models
  ...
  // Sử dụng trực tiếp\u7aef\u70b9Loại\u5bf9\u5e94của URL（\u7edd\u5bf9Đường dẫn，từ\u57dftên\u6839\u62fc\u63a5）
  ...
  const directUrl = extractVideoUrl(submitData);
  if (directUrl) return { url: directUrl, taskId: taskId ? String(taskId) : undefined };
  if (!taskId) throw new Error('\u7edfmộtVideo\u63a5\u53e3Quay lại\u7a7aNhiệm vụ ID');

  // \u8f6e\u8be2：Sử dụng trực tiếp\u7aef\u70b9Loại\u5bf9\u5e94của URL
  ...
  for (let i = 0; i < VIDEO_POLL_MAX_ATTEMPTS; i++) {
    ...
    if (status === 'failed' || status === 'error' || status === 'cancelled') {
      throw new Error(pollData.error?.message || pollData.error || pollData.message || 'VideoTạoThất bại');
    }
  }
...
async function generateVideoViaVolc(
  ...
  // \u9644\u52a0Tải lênHình ảnh（khung hình đầu tiên/\u5c3e\u5e27），Căn chỉnh Director \u9762\u677fcủa callVolcVideoApi
  const grouped = groupVideoUploadFiles(params.uploadFiles);
  ...
  if (!taskId) throw new Error('Volc Quay lại\u7a7aNhiệm vụ ID');
  ...
  for (let i = 0; i < VIDEO_POLL_MAX_ATTEMPTS; i++) {
    ...
    if (status === 'succeeded' || status === 'completed' || status === 'success') {
      ...
      if (!videoUrl) throw new Error('Volc Thành công\u4f46không cóVideo URL');
      return { url: videoUrl, taskId: String(taskId) };
    }
    if (status === 'failed' || status === 'expired' || status === 'cancelled' || status === 'error') {
      throw new Error(pollData.error?.message || pollData.error || 'Volc VideoTạoThất bại');
    }
  }

  throw new Error('Volc VideoTạo\u8d85\u65f6');
}
...
async function generateVideoViaWan(
  ...
  if (!taskId) throw new Error('Wan Quay lại\u7a7aNhiệm vụ ID');
  ...
  for (let i = 0; i < VIDEO_POLL_MAX_ATTEMPTS; i++) {
    ...
    if (status === 'SUCCEEDED' || status === 'COMPLETED') {
      ...
      if (!videoUrl) throw new Error('Wan Thành công\u4f46không cóVideo URL');
      return { url: videoUrl, taskId: String(taskId) };
    }
    if (status === 'FAILED' || status === 'ERROR' || status === 'CANCELLED') {
      throw new Error(pollData.output?.message || pollData.output?.error || 'Wan VideoTạoThất bại');
    }
  }

  throw new Error('Wan VideoTạo\u8d85\u65f6');
}
...
async function generateVideoViaKling(
  ...
  if (!taskId) throw new Error('Kling Quay lại\u7a7aNhiệm vụ ID');
  ...
  for (let i = 0; i < VIDEO_POLL_MAX_ATTEMPTS; i++) {
    ...
    if (status === 'succeed' || status === 'success' || status === 'completed') {
      ...
      if (!videoUrl) throw new Error('Kling Thành công\u4f46không cóVideo URL');
      return { url: videoUrl, taskId: String(taskId) };
    }
    if (status === 'failed' || status === 'error') {
      throw new Error(pollData.data?.task_status_msg || pollData.message || 'Kling VideoTạoThất bại');
    }
  }

  throw new Error('Kling VideoTạo\u8d85\u65f6');
}
...
async function generateVideoViaReplicate(
  ...
  for (let i = 0; i < VIDEO_POLL_MAX_ATTEMPTS; i++) {
    ...
    if (status === 'succeeded') {
      const videoUrl = extractVideoUrl(pollData);
      if (!videoUrl) throw new Error('Replicate Thành công nhưng không Quay lạiVideo URL');
      return { url: videoUrl, taskId: String(predictionId) };
    }
    if (status === 'failed' || status === 'canceled') {
      throw new Error(pollData.error || 'Replicate VideoTạoThất bại');
    }
  }
