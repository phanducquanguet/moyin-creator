...
export async function exportProjectFiles(
  ...
  for (let i = 0; i < filesToDownload.length; i++) {
    ...
    onProgress?.({
      ...
      message: `Tải xuống ${file.filename}`,
    });
...
export async function exportProjectToFolder(
  ...
  try {
    ...
    currentFile++;
    onProgress?.({ current: currentFile, total: totalFiles, message: 'Được viết vào tệp kê khai.json' });
    ...
    if (includeImages && imagesDir) {
      for (const shot of shots) {
        ...
        onProgress?.({ current: currentFile, total: totalFiles, message: `Tải xuống ${filename}` });
    ...
    onProgress?.({ current: totalFiles, total: totalFiles, message: 'XuấtHoàn thành' });
    return true;
  } catch (error) {
...
export async function exportDirectorToFolder(
  ...
  try {
    ...
    // Xuất hình ảnh (khung hình đầu tiên)
    if (includeImages && imagesDir) {
      for (const scene of scenes) {
        ...
        onProgress?.({ current: currentFile, total: totalFiles, message: `Xuấkhung hình đầu tiên ${filename}` });
    ...
    if (includeEndFrames && endFramesDir) {
      for (const scene of scenes) {
        ...
        onProgress?.({ current: currentFile, total: totalFiles, message: `Xuấkhung hình cuối cùng ${filename}` });
