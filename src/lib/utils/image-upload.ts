...
export async function uploadBase64Image(imageData: string): Promise<string> {
  ...
  if (imageData.startsWith('local-image://')) {
    ...
    if (!converted) {
      throw new Error(`Không thể đọc H cục bộình ảnh: ${imageData}`);
    }
  ...
  if (!isImageHostConfigured()) {
    throw new Error('Lưu trữ ảnh chưa được cấu hình');
  }
  ...
  throw new Error(result.error || 'Hình ảnhTải lênThất bại');
}
